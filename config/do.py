...
def create_new_sider(sider_name, sider_title):
    db.sider.insert({'uuid': get_uuid(), 'No': 0,
...
def update_sider(all_sider):
    ...
    for sider_name in new_sider_name_list:
        sider_title = all_sider[sider_name]
        create_new_sider(sider_name, sider_title)
    

    # 把已经存在的做一次更新
    for sider_name in exist_sider_name_list:
        sider_title = all_sider[sider_name]
        update_sider_info(sider_name, sider_title = sider_title)
